# TYUT XCPC 题解与代码仓库

本仓库收录 TYUT XCPC 集训队的区域赛、训练赛与补题资料，包含 AC 代码、题解、外部参考链接与赛后总结。

## 当前收录

- [2025 ICPC Example Regionals / ExampleRegion / The 2025 ICPC Example Regional Contest](solutions/ICPC/Regionals/ExampleRegion/The%202025%20ICPC%20Example%20Regionals/The%202025%20ICPC%20Example%20Regional%20Contest/editorial.md)

## 仓库内容

- 题目 AC 代码
- 作者个人题解
- 比赛总题解与补题索引
- 外部题解与参考资料链接
- 训练记录、坑点总结、模板代码

## 目录约定

统一规范见：

- [仓库结构说明](docs/repo-structure.md)
- [贡献指南](CONTRIBUTING.md)
- [授权与许可说明](docs/licensing.md)

## 许可摘要

- 作者对各自提交内容保留原始著作权。
- 提交到仓库的内容，会按仓库许可与贡献约定被整理、展示与再分发。
- 仓库整体分发采用 CC BY-NC-SA 4.0。
- 单个文件如标注了独立许可，以文件内标注为准。
